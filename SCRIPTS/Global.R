...
#saveRDS(All_Years, file = "./SCRIPTS/dataprepared.rds")
All_Years <- readRDS(file = "./SCRIPTS/dataprepared.rds")
...
All_years_by_months <-  All_Years %>% group_by(Month) %>%
  summarise(T_Active_energy=sum(Global_active_power*1000/60), 
            T_Reactive_energy=sum(Global_reactive_power*1000/60),
            T_Kitchen=sum(Kitchen),
            T_Laundry=sum(Laundry), 
            T_AC_Heater=sum(AC_Heater))
